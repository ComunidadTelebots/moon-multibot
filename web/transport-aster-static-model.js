...
export function createAsterStaticModelLoader({THREE:T,qualityLevel=2,url,fetchImpl=fetch}={}){
  const profile=qualityLevel>=3?"ULTRA":"HIGH",modelUrl=url||(qualityLevel>=3?"./models/aster-viento-ultra.json":"./models/aster-viento-high.json");
  let disposed=false,active=null;
  async function apply(exterior){
    if(qualityLevel<2||!exterior||disposed)return false;
    const response=await fetchImpl(modelUrl);if(!response.ok)throw new Error(`Aster static model HTTP ${response.status}`);const data=await response.json();
    ...
    const group=new T.Group();group.name=`aster_viento_static_${profile.toLowerCase()}_body`;group.userData.source=data.source;group.userData.profile=profile;
  ...
  return{apply,dispose,profile,modelUrl,get active(){return active;}};
}
