...
export function createBakedMaterialLibrary({THREE:T,qualityLevel=2,exteriorUrl="./generated-textures/aster-exterior-material-atlas-v1.png",cabinUrl="./generated-textures/aster-cabin-material-atlas-v1.png"}={}){
  ...
  function materialFor(base,atlas,tile,key){const cacheKey=`${base.uuid}:${key}`;if(materials.has(cacheKey))return materials.get(cacheKey);const material=base.clone(),map=atlas.clone(),[column,row]=tile,{repeat,offset}=atlasTileTransform(column,row);map.repeat.set(...repeat);map.offset.set(...offset);map.needsUpdate=true;material.map=map;material.needsUpdate=true;materials.set(cacheKey,material);return material;}
