...
export function createEmergencyFleet({ THREE:T, qualityLevel=2, atlasUrl }) {
  ...
  function create(type){const definition=DEFINITIONS[type];if(!definition)throw new Error(`Unknown emergency vehicle ${type}`);const root=new T.Group();root.name=`drivable_${type}`;({ambulance,fire,recovery})[type](root);root.userData.definition=definition;root.userData.cockpitOffset=definition.cockpit;root.userData.maxSpeed=definition.maxSpeed;return root;}
  ...
  function dispose(){resources.forEach(r=>r.dispose());Object.values(mats).forEach(m=>{m.map?.dispose();m.dispose();});}
