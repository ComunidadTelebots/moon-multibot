...
MANIFEST=tuple({"id":fid,"title":f"{prefix} {LABELS[r]} en Moonbot","capability":f"{prefix} {LABELS[r]}","module":"resource_federation_continuity_assistance_engines.py","api":api.__name__,"roles":("master",f"{scope}:{r}"),"preflight":f"repo_scan_before:{fid}/{api.__name__}: ID, API y capacidad exacta ausentes; contrato {family} específico no existente.","test":f"tests/test_resource_federation_continuity_assistance.py::test_{fid.replace('-','_')}"} for i,(fid,r,api) in enumerate(zip(IDS,RESOURCES,ALL_APIS)) for family,prefix,scope in (fam(i),))
