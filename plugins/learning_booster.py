...
def handle_command(bot, cid, uid, text, rank):
    ...
    if t_lower.startswith("/sentiment"):
        ...
        bot.send_msg(cid, f"📊 **Análisis de Sentimiento**: {sentiment} (Polaridad: {round(polarity, 2)}, Subjetividad: {round(subjectivity, 2)})")
        return True
    
    elif t_lower == "/boost_learning":
        if rank not in ["admin", "master"]:
            bot.send_msg(cid, "❌ Solo administradores pueden boostear aprendizaje.")
            return True
        ...
        if positive_phrases:
            ...
            bot.send_msg(cid, f"🚀 **Aprendizaje Potenciado**: {len(positive_phrases)} frases positivas aprendidas.")
        else:
            bot.send_msg(cid, "❌ No hay frases positivas recientes para aprender.")
